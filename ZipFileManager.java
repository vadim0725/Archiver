...
public class ZipFileManager {
    ...
    public void extractAll(Path outputFolder) throws Exception {
        ...
        try(ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(zipFile))) {
            ZipEntry zipEntry = zipInputStream.getNextEntry();
            while (zipEntry != null) {
                Path resolvedPath = outputFolder.resolve(Paths.get(zipEntry.getName()));
                if (Files.notExists(resolvedPath.getParent())) {
                    Files.createDirectories(resolvedPath.getParent());
                }
                try(OutputStream extractedZipFile = Files.newOutputStream(resolvedPath)) {
                    copyData(zipInputStream, extractedZipFile);
                }
                zipEntry = zipInputStream.getNextEntry();
            }
        }
